...
function Get-Link {
    ...
    switch ($mirror) {
        $true { return "https://spotifycrack-official.github.io/spotifycrack" + $endlink }
        default { return "https://raw.githubusercontent.com/spotifycrack-Official/spotifycrack/main" + $endlink }
    }
}
...
New-Item -Type Directory -Name "spotifycrack_Temp-$(Get-Date -UFormat '%Y-%m-%d_%H-%M-%S')" | Convert-Path | Set-Location
...
function extract ($counts, $method, $name, $helper, $add, $patch) {
    switch ( $counts ) {
        ...
        "more" {  
            ...
            $xpui_spa_patch = Join-Path (Join-Path $env:APPDATA 'Spotify\Apps') 'xpui.spa'
            $zip = [System.IO.Compression.ZipFile]::Open($xpui_spa_patch, 'update') 
            $zip.Entries | Where-Object { $_.FullName -like $name -and $_.FullName.Split('/') -notcontains 'spotifycrack-helper' } | foreach { 
...
if ($test_js) {
    ...
    if ($ch -eq 'y') { 
        $Url = "https://telegra.ph/spotifycrack-FAQ-09-19#Can-I-use-spotifycrack-and-Spicetify-together?"
        Start-Process $Url
    }
...
If ($test_spa) {
    ...
    $patched_by_spotifycrack = $reader.ReadToEnd()
    $reader.Close()

    If ($patched_by_spotifycrack -match 'patched by NoxxyITA') {
        $zip.Dispose()    
    ...
    # Forced exp
    extract -counts 'one' -method 'zip' -name 'xpui.js' -helper 'ForcedExp' -add $webjson.others.byspotifycrack.add
    ...
    if ($podcast_off -or $adsections_off) {
        ...
        if ($section -ne $null) {
            ...
            injection -p $xpui_spa_patch -f "spotifycrack-helper" -n "sectionBlock.js" -c $section
        }
        else {
    ...
    if ($urlform_goofy -and $idbox_goofy) {
        ...
        if ($goofy -ne $null) {
            ...
            injection -p $xpui_spa_patch -f "spotifycrack-helper" -n "goofyHistory.js" -c $goofy
        }
    }
    ...
    if ($lyrics_stat) {
        ...
        injection -p $xpui_spa_patch -f "spotifycrack-helper/lyrics-color" -n @("rules.css", "colors.css") -c @($rulesContent, $colorsContent) -i "rules.css"
